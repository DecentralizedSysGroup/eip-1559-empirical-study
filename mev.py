...
def calc_FBB():
    ...
    while blockno > block_start:
        ...
        content = requests.get(url,params={'before': str(blockno),'limit':'4'}).text
        true = True
        false = False
        null = None
        mp = eval(content)
        ...
        for block in mp['blocks']:
            ...
            FBB_coinbase_transfer[thisblockno-block_start] = int(block['coinbase_transfers'])
            FBB_gas_fee[thisblockno-block_start] = int(block['miner_reward']) - int(block['coinbase_transfers'])
...
def calc_basic():
    ...
    for blockno in range(block_start,block_end):
        ...
        if blockno < 7280000: assert(0) #Constantinople fork, changing the static reward from 3 eth to 2 eth
        static_reward[blockno-block_start] = 2*10**18
        uncle_incl_reward[blockno-block_start] = len(block_info['uncles'])*625*10**14
        ...
        for tx in block_info['transactions']:
            ...
            if txtype==HexBytes('0x0') or txtype==HexBytes('0x1'):
                ...
                if type(maxPriorityFeePerGas)==str: maxPriorityFeePerGas = int(maxPriorityFeePerGas,16)
                maxFeePerGas = tx['maxFeePerGas']
                if type(maxFeePerGas)==str: maxFeePerGas = int(maxFeePerGas,16)
                tx_gasfee = min(maxPriorityFeePerGas,maxFeePerGas-basefee)*recepit['gasUsed']
            else:
                print('txtype =',txtype)
...
def MEVdata_to_csv(stepsize:int):
    ...
    writer.writerow(('block_number','FBB_coinbase_transfer','FBB_gas_fee','non_FBB_gas_fee','static_reward','uncle_incl_reward'))
    for start in range(block_start,block_end,stepsize):
        print(start)
        end = min(start+stepsize,block_end)
        file_prefix = './MEVdata/[%d,%d)'%(start,end)
        FBB_coinbase_transfer = read_list(file_prefix+'FBB_coinbase_transfer.txt')
        ...
        for blockno in range(start,end):
            ...
            id = blockno - start
            writer.writerow((blockno,FBB_coinbase_transfer[id],FBB_gas_fee[id],non_FBB_gas_fee[id],static_reward[id],uncle_incl_reward[id]))
...
def csv_to_img(bunch_size=10000):
    ...
    for row in reader:
        ...
        all = (float(row[1])+float(row[2])+float(row[3])+0+float(row[5]))/10**18
    ...
    fig = sns.histplot(np.array(x_before),stat='density',color='#1f77b4',bins=100,label='pre London Hardfork',binrange=(0,2),edgecolor='none')
    #fig.legend(loc='upper center', ncol=2)
    fig = sns.histplot(np.array(x_after),stat='density',color='#ff7f0e',bins=100,label='post London Hardfork',binrange=(0,2),edgecolor='none')
    ...
    plt.ylim(0,40)
    fig = sns.histplot(np.array(FBB_before),stat='density',color='#1f77b4',bins=100,label='pre London Hardfork',binrange=(0,.4),edgecolor='none')
    #fig.legend(loc='upper center', ncol=2)
    fig = sns.histplot(np.array(FBB_after),stat='density',color='#ff7f0e',bins=100,label='post London Hardfork',binrange=(0,.4),edgecolor='none')
    fig.legend(loc='upper center',ncol=1,fontsize=fontsize)
    ...
    plt.xlim(0,2)
    plt.xticks([0.,.5,1.,1.5,2.])
    fig = sns.histplot(np.array(non_before),stat='density',color='#1f77b4',bins=100,label='pre London Hardfork',binrange=(0,2),edgecolor='none')
    #fig.legend(loc='upper center', ncol=2)
    fig = sns.histplot(np.array(non_after),stat='density',color='#ff7f0e',bins=100,label='post London Hardfork',binrange=(0,2),edgecolor='none')
    fig.legend(loc='upper center',ncol=1,fontsize=fontsize)
    plt.gcf().subplots_adjust(left=.15,top=.97,bottom=0.13) 
    ...
    ticks = [block_start,london_fork,block_end] if london_fork >= block_start and london_fork < block_end else [block_start,block_end]
    ...
    bottom = [0] * bar_cnt
    width = bunch_size
    ax.bar(labels,static_reward,bottom=bottom,width=width,label='static reward',align='edge',color=col[0])
    for i in range(len(bottom)): bottom[i] += static_reward[i]
    ax.bar(labels,uncle_incl_reward,bottom=bottom,width=width,label='uncle inclusion reward',align='edge',color=col[1])
    for i in range(len(bottom)): bottom[i] += uncle_incl_reward[i]
    ax.bar(labels,non_FBB_gas_fee,bottom=bottom,width=width,label='non-FBB gas fee',align='edge',color=col[2])
    for i in range(len(bottom)): bottom[i] += non_FBB_gas_fee[i]
    ax.bar(labels,FBB_gas_fee,bottom=bottom,width=width,label='FBB gas fee',align='edge',color=col[3])
    for i in range(len(bottom)): bottom[i] += FBB_gas_fee[i]
    ax.bar(labels,FBB_coinbase_transfer,bottom=bottom,width=width,label='FBB coinbase transfer',align='edge',color=col[4])
    for i in range(len(bottom)): bottom[i] += FBB_coinbase_transfer[i]
    ...
    for i in range(bar_cnt):
        all = static_reward[i]+uncle_incl_reward[i]+non_FBB_gas_fee[i]+FBB_gas_fee[i]+FBB_coinbase_transfer[i]
        ratio_FBB_gasfee[i] = FBB_gas_fee[i]/all
        ratio_FBB_coinbase_transfer[i] = FBB_coinbase_transfer[i]/all
    bottom = [0] * bar_cnt
    width = bunch_size
    ax.bar(labels,ratio_FBB_gasfee,bottom=bottom,width=width,label='FBB gas fee',align='edge',color=col[3])
    for i in range(len(bottom)): bottom[i] += ratio_FBB_gasfee[i]
    ax.bar(labels,ratio_FBB_coinbase_transfer,bottom=bottom,width=width,label='FBB coinbase transfer',align='edge',color=col[4])
    for i in range(len(bottom)): bottom[i] += ratio_FBB_coinbase_transfer[i]
    ax.legend(loc='upper center',ncol=1,fontsize=fontsize)
    plt.gcf().subplots_adjust(left=.15,top=.97,bottom=0.13) 
    ...
    width = bunch_size
    for i in range(bar_cnt):
        all = uncle_incl_reward[i]+non_FBB_gas_fee[i]+FBB_gas_fee[i]+FBB_coinbase_transfer[i]
        ratio_FBB_gasfee[i] = FBB_gas_fee[i]/all
        ratio_FBB_coinbase_transfer[i] = FBB_coinbase_transfer[i]/all
    bottom = [0] * bar_cnt
    width = bunch_size
...
if __name__=='__main__':
    ...
    if option == '--data':
        ...
        write_list(FBB_coinbase_transfer,file_prefix+'FBB_coinbase_transfer.txt')
